...
export async function getDailyMetals({ force = false } = {}) {
  ...
  if (!force && isFresh(goldCached) && isFresh(silverCached))
    return { gold: goldCached, silver: silverCached };
  if (!force && inFlight) return inFlight;
  if (typeof navigator !== "undefined" && navigator.onLine === false) {
    if (goldCached || silverCached)
      return { gold: goldCached, silver: silverCached };
  }
  inFlight = (async () => {
    const url = `${API_BASE.replace(
      /\/$/,
      ""
    )}/latest?api_key=${encodeURIComponent(API_KEY)}&base=${encodeURIComponent(
      CURRENCY
    )}&currencies=XAU,XAG`;
    ...
    if (!Number.isFinite(xau) || !Number.isFinite(xag))
      throw new Error("Respuesta sin tasas XAU/XAG válidas");
    ...
    const silver = {
      ...
      currency: CURRENCY,
      priceOz: priceOzXAG,
      priceGram: priceOzXAG / OZ_TO_G,
      fetchedAt: now,
    };
    try {
      saveRecord("gold", gold);
      saveRecord("silver", silver);
    } catch {}
    return { gold, silver };
  })();
  try {
...
export function getGoldHistory() {
  return readHistory("gold");
}
export function getSilverHistory() {
  return readHistory("silver");
}
export function clearGoldCache() {
  localStorage.removeItem(keys("gold").daily);
}
export function clearSilverCache() {
  localStorage.removeItem(keys("silver").daily);
}

const env = (k, fb = "") => {
  const v = typeof import.meta !== "undefined" ? import.meta.env : {};
  const viteVal = v && (v[`VITE_${k}`] ?? v[k]);
  if (viteVal !== undefined) return viteVal;
  const pe = typeof process !== "undefined" ? process.env : {};
  const craVal = pe && (pe[`REACT_APP_${k}`] ?? pe[k]);
  if (craVal !== undefined) return craVal;
  return fb;
};
...
const isFresh = (rec) =>
  ...
  Date.now() - rec.fetchedAt < TTL_H * 3600_000;

function saveRecord(metal, record) {
  const k = keys(metal);
  localStorage.setItem(k.daily, JSON.stringify(record));
  const history = JSON.parse(localStorage.getItem(k.hist) || "[]").filter(
...
function readDaily(metal) {
  try {
    return JSON.parse(localStorage.getItem(keys(metal).daily) || "null");
  } catch {
    return null;
  }
}
function readHistory(metal) {
  try {
    return JSON.parse(localStorage.getItem(keys(metal).hist) || "[]").sort(
